...
/*  82 */     double kA = 0.0D;
/*     */ 
/*  84 */     double kB = 0.2026423672846756D;
...
/*     */     double kA;
/*     */     double kB;
/*  99 */     if (thetaSq < 1.0E-08D) {
/* 100 */       double kA = 1.0D - 0.16666667163372D * thetaSq;
/* 101 */       kB = 0.5D;
/*     */     }
/*     */     else
/*     */     {
/*     */       double kA;
/* 103 */       if (thetaSq < 1.0E-06D) {
/* 104 */         double kB = 0.5D - 0.0416666679084301D * thetaSq;
/* 105 */         kA = 1.0D - thetaSq * 0.16666667163372D * (1.0D - 0.16666667163372D * thetaSq);
/*     */       } else {
...
/*     */   {
/* 122 */     double cosAngle = (so3.get(0, 0) + so3.get(1, 1) + so3.get(2, 2) - 1.0D) * 0.5D;
/*     */ 
/* 124 */     result.set((so3.get(2, 1) - so3.get(1, 2)) / 2.0D, (so3.get(0, 2) - so3.get(2, 0)) / 2.0D, (so3.get(1, 0) - so3.get(0, 1)) / 2.0D);
/*     */ 
...
/*     */       {
/* 150 */         r2.set(d0, (so3.get(1, 0) + so3.get(0, 1)) / 2.0D, (so3.get(0, 2) + so3.get(2, 0)) / 2.0D);
/*     */       }
/* 152 */       else if (d1 * d1 > d2 * d2)
/*     */       {
/* 154 */         r2.set((so3.get(1, 0) + so3.get(0, 1)) / 2.0D, d1, (so3.get(2, 1) + so3.get(1, 2)) / 2.0D);
/*     */       }
/*     */       else
/*     */       {
/* 158 */         r2.set((so3.get(0, 2) + so3.get(2, 0)) / 2.0D, (so3.get(2, 1) + so3.get(1, 2)) / 2.0D, d2);
/*     */       }
/*     */ 
